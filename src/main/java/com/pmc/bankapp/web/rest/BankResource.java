...
/**
 * REST controller for managing {@link com.pmc.bankapp.domain.Bank}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Bank> partialUpdateBank(@PathVariable(value = "id", required = false) final Long id, @RequestBody Bank bank)
        ...
        Optional<Bank> result = bankRepository
            ...
            .map(existingBank -> {
                ...
                if (bank.getTransactionCharge() != null) {
                    existingBank.setTransactionCharge(bank.getTransactionCharge());
                }
